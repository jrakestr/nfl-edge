...
- ops-lines (this change): `ops/lines-only.sh` runs `nfl-edge ingest --season 2026 --lines-only` then `nfl-edge lines --season 2026 --week 1` (newest sim run, all games). Both steps append to `output/cron-lines.log`. Plist unchanged (`StartInterval` 1800); a `git pull` on the Mac is enough — no overlap swap. The Edge board no longer shows a "run nfl-edge lines to refresh" banner; per-game "moved since sim" stays.
