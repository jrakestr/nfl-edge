...
- DK salary CSVs live in gitignored `data/dk/`. Player-ID joins go through `raw.players` and `raw.dk_player_crosswalk` (LAR→LA, JAC→JAX, WSH→WAS). DFS is slate-scoped (main/full/showdown have distinct DK IDs); upload CSVs must use that slate's IDs. Line 1 of every DK upload file is the site header (`QB,RB,RB,WR,WR,WR,TE,FLEX,DST` or the showdown CPT header) — never a comment. Provenance is the filename `dk_upload_<slate_id>_<run_id first 8>_<source>.csv` (`sim` or `user-optimized`); the CLI path `data/dfs/<run_id>/<site>/<slate>/` also carries run and slate. `ops/week-rebuild.sh` discovers slates from the CSVs on disk for that week rather than hardcoding main/full. Status O/D/Q on ingest writes `raw.player_overrides` (week-keyed, so the last DK Status merge is week-wide); a blank DK Status may clear only DK-sourced rows, never manual or claims-promote rows. The UI and the DFS pipeline drop anyone whose current override is OUT, IR, or D set after the run's `created_at`; the Fpts floor is not an injury filter.
